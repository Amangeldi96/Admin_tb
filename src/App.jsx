...
function AdminApp({authUser}){const[page,setPage]=useState('dashboard'),[notice,setNotice]=useState(''),[error,setError]=useState(''),[globalSearch,setGlobalSearch]=useState('');const[ads,setAds]=useState([]),[vipAds,setVipAds]=useState([]),[requests,setRequests]=useState([]),[users,setUsers]=useState([]),[logs,setLogs]=useState([]),[reports,setReports]=useState([]);const[blockTarget,setBlockTarget]=useState(null),[deleteTarget,setDeleteTarget]=useState(null),[clearTarget,setClearTarget]=useState(null),[reason,setReason]=useState('Эрежени бузган'),[days,setDays]=useState(3),[notificationPermission,setNotificationPermission]=useState(()=>typeof Notification==='undefined'?'unsupported':Notification.permission);
