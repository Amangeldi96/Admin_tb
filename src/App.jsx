...
function AdminApp({authUser}){const[page,setPage]=useState('dashboard'),[notice,setNotice]=useState(''),[error,setError]=useState(''),[globalSearch,setGlobalSearch]=useState('');const[ads,setAds]=useState([]),[vipAds,setVipAds]=useState([]),[requests,setRequests]=useState([]),[users,setUsers]=useState([]),[logs,setLogs]=useState([]),[reports,setReports]=useState([]);const[blockTarget,setBlockTarget]=useState(null),[deleteTarget,setDeleteTarget]=useState(null),[reason,setReason]=useState('Эрежени бузган'),[days,setDays]=useState(3),[notificationPermission,setNotificationPermission]=useState(()=>typeof Notification==='undefined'?'unsupported':Notification.permission);
...
useEffect(()=>{
  ...
  const stops=[];
  const watch=(name,label)=>stops.push(onSnapshot(collection(db,name),snap=>{
    ...
    localStorage.setItem(seenKey,JSON.stringify(Array.from(seen).slice(-300)));
  }));
  watch('ads','Жаңы жарнама келди');
  watch('vip_ads','Жаңы VIP баннер келди');
  watch('vip_requests','Жаңы VIP сурам келди');
...
const clearHistory=async()=>{if(!window.confirm('Аракеттер тарыхын толугу менен тазалайсызбы?'))return;try{await Promise.all(logs.map(x=>deleteDoc(doc(db,'admin_logs',x.id))));flash('Тарых толугу менен тазаланды')}catch(e){flash(`Тазалоодо ката: ${e.message}`)}};
const clearViolations=async()=>{if(!window.confirm('Эреже бузууларды толугу менен тазалайсызбы?'))return;try{await Promise.all(reports.map(x=>deleteDoc(doc(db,'reports',x.id))));flash('Эреже бузуулар толугу менен тазаланды')}catch(e){flash(`Тазалоодо ката: ${e.message}`)}};
...
// Parent document өчүрүлгөндө Firestore subcollection автоматтык өчүрбөйт.
// Ошондуктан ads/{id}/viewers документтерин өзүнчө толук тазалайбыз.
const deleteSubcollectionDocs=async(parentCollection,parentId,subcollectionName)=>{
  const snap=await getDocs(collection(db,parentCollection,parentId,subcollectionName));
...
const confirmDelete=async()=>{
  ...
  try{
    ...
    // Негизги ads документи өчкөндө viewers калдыгы калбасын.
    if(item._collection==='ads'){
      // Бул жарнамага тиешелүү бардык белгилүү subcollection'дарды тазалайбыз.
      await deleteSubcollectionDocs('ads',item.id,'viewers');
      await deleteSubcollectionDocs('ads',item.id,'rating_favorites');

      // Колдонуучулардын users/{uid}/favorites/{adId} көчүрмөлөрүн да өчүрөбүз.
      // Ошондо админ өчүргөн жарнама "Тандалгандар" бетинде калбайт.
      const usersSnap=await getDocs(collection(db,'users'));
      let favBatch=writeBatch(db);
      let favBatchCount=0;

      for(const userSnap of usersSnap.docs){
        const favRef=doc(db,'users',userSnap.id,'favorites',item.id);
        const favSnap=await getDoc(favRef);

        if(favSnap.exists()){
          favBatch.delete(favRef);
          favBatchCount++;

          if(favBatchCount>=400){
            await favBatch.commit();
            favBatch=writeBatch(db);
            favBatchCount=0;
          }
        }
      }

      if(favBatchCount>0)await favBatch.commit();
    }
...
const block=async()=>{try{await setDoc(doc(db,'users',blockTarget.id),{status:'blocked',blockedUntil:Date.now()+Number(days)*86400000,blockReason:reason,warningCount:increment(1),updatedAt:serverTimestamp()},{merge:true});setBlockTarget(null);flash('Колдонуучу блоктолду')}catch(e){flash(`Ката: ${e.message}`)}};const unblock=async u=>{try{await setDoc(doc(db,'users',u.id),{status:'active',blockedUntil:null,blockReason:null,updatedAt:serverTimestamp()},{merge:true});await writeLog('Колдонуучу блоктон чыгарылды',u.displayName||u.name||u.email||u.id);flash('Колдонуучу блоктон чыгарылды')}catch(e){flash(`Ката: ${e.message}`)}};
